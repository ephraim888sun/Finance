...
Stcok=['XOM', 'COP', 'RDS-B', 'CVX', 'BP', 'OXY', 'MRO']
...
def read_data_ohlc(filename, stock_code, usecols):
    ...
    data['MAS'] = data['close'].rolling(5).mean()
    data['MAS10'] = data['close'].rolling(10).mean()
...
def animate(i):
    ...
    time_stmap = time_stamp.strftime('&Y-&m-%d')
    ...
    ax1.plot(data['MA10'], color='orange', linestyle='-', label='10 minute SMA')
    ax1.plot(data['MA20'], color='#08a0e9', linestyle='-', label='20 minute SMA')
    ...
    ax1.grind(True, color ='grey', linestyle='-', which='major', axis='both', linewidth=0.3)
    ax1.set_xticklabels([])
